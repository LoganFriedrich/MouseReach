"""
Generate per-video FP/FN review manifests for v8.0.3 + asymmetric tolerance
+ matcher-aware topology classifier.

Successor to generate_fpfn_review_manifests_v8_0_3.py (which used the OLD
connected-components-only topology classifier). Differences:

- Same v8.0.3 algo + asymmetric matcher.
- NEW topology classifier (per locked rules 2026-05-22):
  * 1:1 matched -> TP
  * 1:1 overlap-only -> TOLERANCE_ERROR
  * 1:N (N>=2) -> MERGED (all events in component labeled MERGED)
  * N:1 (N>=2) -> FRAGMENTED
  * N:M (N>=2, M>=2): per-event labels
    - matched pair -> TP
    - unmatched FP overlapping unmatched GT in same comp -> TOLERANCE_ERROR
    - unmatched FN overlapping unmatched algo in same comp -> TOLERANCE_ERROR
    - stranded -> FALSE_POSITIVE / FALSE_NEGATIVE
  * No COMPLEX label.
- Output: fpfn_review_manifests/v8.0.3/ (overwrites the OLD-topology version).
- Archive: existing v8.0.3/ (OLD topology) archived to
  v8.0.3_archive/<timestamp>_pre_matcher_aware_topology/.
...
CAL_SNAPSHOT_NAME = "v8.0.3_via_trim_and_apex_split_on_v8.0.1_model_3_1_baseline_loocv"
...
GEN_SNAPSHOT_NAME = "v8.0.3_via_trim_and_apex_split_on_v8.0.0_holdout_generalization_merge_gap_0"
...
# v8.0.3 apex-split parameters (production defaults, calibrated 2026-05-22)
...
def apply_v803_apex_split(algos: List[Tuple[int, int]], norm_pos: np.ndarray
                           ) -> List[Tuple[int, int]]:
    """Apply v8.0.3 apex-split-at-trough to a list of (start, end) reaches."""
    spans = [ReachSpan(start_frame=s, end_frame=e) for s, e in algos]
...
# Position cols needed for v8.0.3 norm_pos (hand-centroid-to-BoxL distance).
...
def _build_calibration_per_video() -> Dict[str, Dict[str, Any]]:
    ...
    # Load paw_mean_lk AND positions (for norm_pos) from parquet
    print(f"Loading parquet for paw_mean_lk and positions...", flush=True)
    ...
    for vid, grp in df.groupby("video_id", sort=False):
        ...
        lk_arr[g["frame"].to_numpy()] = g["paw_mean_lk"].to_numpy()
        lk_by_vid[vid] = lk_arr
        # Build a DLC-style df with bodypart_x/y columns expected by
...
def _build_holdout_per_video() -> Dict[str, Dict[str, Any]]:
    ...
    for algo_path in sorted(GEN_ALGO_DIR.glob("*_reaches.json")):
        ...
        # Load DLC for paw_mean_lk AND norm_pos
...
def _topology_for_event(algo_key: Optional[Tuple[int, int]],
                        ...
                        matched_pair_keys: Optional[set] = None,
                        ) -> Tuple[str, Optional[str], Optional[int]]:
    """Matcher-aware topology classifier (locked rules 2026-05-22).

    Replaces the old connected-components-only classifier.

    Per-event labels:
      - 1:1 component: TP if matcher matched, TOLERANCE_ERROR if overlap-only
      - 1:N (N>=2): MERGED for all events in component (absorbs matched pair)
      - N:1: FRAGMENTED
      - N:M (N>=2, M>=2): decompose per event
          * matched pair (event_kind=TP) -> TP
          * unmatched FP with overlapping unmatched GT in same comp -> TOLERANCE_ERROR
          * unmatched FN with overlapping unmatched algo in same comp -> TOLERANCE_ERROR
          * stranded -> FALSE_POSITIVE / FALSE_NEGATIVE

    matched_pair_keys: set of (algo_key, gt_key) tuples for matched pairs.
    """
    ...
    n_algo = len(a_list); n_gt = len(g_list)
    ...
    algo_keys_in_comp = {(a.start_frame, a.end_frame) for a in a_list}
    gt_keys_in_comp = {(g.start_frame, g.end_frame) for g in g_list}
    matched_a_in_comp = {ak for ak, gk in matched_pair_keys
                          if ak in algo_keys_in_comp and gk in gt_keys_in_comp}
    matched_g_in_comp = {gk for ak, gk in matched_pair_keys
                          if ak in algo_keys_in_comp and gk in gt_keys_in_comp}
    unmatched_a = algo_keys_in_comp - matched_a_in_comp
    unmatched_g = gt_keys_in_comp - matched_g_in_comp
    if event_kind == "FP":
        # algo_key is this event's algo; check if it overlaps any unmatched GT
        a_s, a_e = algo_key
...
def _build_event(rec: Dict[str, Any],
    ...
    if kind == "TP":
        ...
        topo, sub, cid = _topology_for_event(
            ...
            event_kind="TP", matched_pair_keys=matched_pair_keys)
        return {
            "kind": "TP",
    ...
    if kind == "FP":
        ...
        topo, sub, cid = _topology_for_event(
            ...
            event_kind="FP", matched_pair_keys=matched_pair_keys)
        return {
            "kind": "FP",
            "detector": {"start": a_s, "end": a_e},
            "gt": None,
    ...
    if kind == "FN":
        ...
        topo, sub, cid = _topology_for_event(
            ...
            event_kind="FN", matched_pair_keys=matched_pair_keys)
        return {
            "kind": "FN",
            "detector": None,
            "gt": {"start": g_s, "end": g_e},
...
def build_manifests(per_video: Dict[str, Dict[str, Any]],
    ...
    for video_id, vd in per_video.items():
        ...
        # Matched pair keys (for new classifier) + matched GT keys (for split_twin)
        matched_gt_keys = set()
        matched_pair_keys = set()
        for rec in records:
            if rec["kind"] == "TP":
                gt_k = (int(rec["gt_start"]), int(rec["gt_end"]))
                algo_k = (int(rec["algo_start"]), int(rec["algo_end"]))
                matched_gt_keys.add(gt_k)
                matched_pair_keys.add((algo_k, gt_k))
        ...
        for ev in events:
            topology_summary[ev["topology"]] += 1
        # Collapse MERGED/FRAGMENTED to component-level counts (each MERGED
        # component has multiple events that all carry the same MERGED label
        # under the new rule; we want to report 1 MERGED per component).
        merged_cids = {ev["component_id"] for ev in events
                       if ev["topology"] == "MERGED"}
        frag_cids = {ev["component_id"] for ev in events
                     if ev["topology"] == "FRAGMENTED"}
        topology_summary["MERGED"] = len(merged_cids)
        topology_summary["FRAGMENTED"] = len(frag_cids)
        # TOLERANCE_ERROR in N:M decomposed cases gets emitted on both the
        # FP-side and FN-side event of each soft pair; collapse to pairs.
        # Count distinct (component_id, algo, gt) soft-pairs:
        tol_pair_events = [ev for ev in events
                           if ev["topology"] == "TOLERANCE_ERROR"]
        # Each 1:1 TOLERANCE_ERROR contributes 2 events (1 FP + 1 FN).
        # Each N:M soft-pair similarly contributes 2 events.
        # So divide by 2 for the human-facing count.
        topology_summary["TOLERANCE_ERROR"] = len(tol_pair_events) // 2
...
def write_manifests(manifests: Dict[str, Dict[str, Any]],
    ...
    for video_id, manifest in manifests.items():
        ...
        out_path.write_text(json.dumps(manifest, indent=2, default=int),
                             encoding="utf-8")
...
def _print_summary(manifests, corpus_label):
    ...
    for video_id, m in manifests.items():
        ...
        print(f"    {video_id:35} unfilt TP={n_tp:>4} FP={n_fp:>4} FN={n_fn:>4}  |  "
              ...
              f"(trim_drop={m.get('v802_trim_dropped', 0)} apex_split={m.get('v803_apex_splits_added', 0)})")
...
def main():
    print("=" * 70)
    print("Generate FP/FN review manifests -- v8.0.3 + asymmetric tolerance")
    ...
    print(f"Apex:    prominence={APEX_PROMINENCE}, depth_min={APEX_DEPTH_MIN}, "
          f"peak2_rel_max={APEX_PEAK2_REL_MAX}, min_distance={APEX_MIN_DISTANCE}, "
          f"min_span={APEX_MIN_SPAN}")
    print("=" * 70)
    print()

    print("Archiving prior v8.0.2 manifests + any prior v8.0.3 rerun...")
