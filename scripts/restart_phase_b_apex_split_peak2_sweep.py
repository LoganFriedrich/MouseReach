"""
v8.0.x experiment: apex-split peak2_rel sweep at prom=0.12, depth=0.5.

Final sub-sweep on the apex-split parameter triplet. Pinned configs from
prior sweeps: prominence=0.12 (sweet spot), depth_min=0.5 (winner of depth
sweep -- identical to 0.4 at this prom, drops MERGED catches above 0.5).
This run varies peak2_rel cutoff.

peak2_rel semantics: if the SECOND peak in the algo span is at >=cutoff
of the span length, the apex split is suppressed. Rationale: at the very
end of the algo span, peaks usually indicate end-of-reach grab/hold/jitter,
not a real second reach.
...
3. Unverified hypotheses:
   - That tighter peak2_rel (e.g. 0.75 or 0.70) further reduces
     over-splits at the cost of missing some real second-reach catches
     where peak2 lands close to span end.
   - That looser peak2_rel (e.g. 0.90) catches a few more MERGED but
     also introduces more end-of-reach over-splits.

4. FN-direction-reporting: lead with FN delta vs cumulative best.
   Pair legacy with topology.

5. Framework: output to v8.0.2_dev_apex_split_peak2_sweep/.

6. Branch + tag: feature/v8-apex-split-postprocess +
   v8-pre-apex-split-2026-05-22.

7. Decision rule: same as prior sub-sweeps. Best config = highest MERGED
   catch with lowest over-split count, FRAG <= 2x baseline.
"""
...
OUT_DIR = Path(
    r"Y:\2_Connectome\Behavior\MouseReach_Improvement\Improvement_Snapshots"
    r"\reach_detection\v8.0.2_dev_apex_split_peak2_sweep"
)
...
PEAK2_REL_VALUES = [0.70, 0.75, 0.80, 0.85, 0.90]
...
def apex_split(reach, norm_pos, peak2_rel_max,
               prominence=PROMINENCE, depth_min=DEPTH_MIN,
...
def score_corpus(corpus_data, peak2_rel_max, apply_apex):
    ...
    for vid, vd in corpus_data.items():
        ...
        if apply_apex:
            ...
            for reach in algos_v802:
                parts = apex_split(reach, vd["norm_pos"], peak2_rel_max=peak2_rel_max)
                if len(parts) > 1:
...
def main():
    print("=" * 70)
    print(f"APEX SPLIT PEAK2_REL SWEEP (prom={PROMINENCE}, depth={DEPTH_MIN})")
    print("=" * 70)
    print()
    ...
    cal_base = score_corpus(cal_data, peak2_rel_max=0.85, apply_apex=False)
    hol_base = score_corpus(hol_data, peak2_rel_max=0.85, apply_apex=False)
    ...
    results = {}
    for p2 in PEAK2_REL_VALUES:
        print(f"--- peak2_rel<{p2} ---", flush=True)
        cal_r = score_corpus(cal_data, peak2_rel_max=p2, apply_apex=True)
        hol_r = score_corpus(hol_data, peak2_rel_max=p2, apply_apex=True)
    ...
    print("=" * 150)
    print(f"SUMMARY: peak2_rel sweep at prom={PROMINENCE}, depth={DEPTH_MIN} (LIVE GT)")
    print("=" * 150)
    print(f"{'peak2':>6}  {'CAL TP':>7} {'dTP':>4} {'FP':>4} {'dFP':>4} {'FN':>5} {'dFN':>4}  "
