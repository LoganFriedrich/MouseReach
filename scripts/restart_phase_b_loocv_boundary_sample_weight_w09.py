...
training time -- fourth variant (w=0.9). Confirming sweet-spot.

BSW sweep so far (vs baseline TP=1918 FP=337 FN=457 exact_start=83.47%):
  w=0.5: TP=1927 (+9)  FP=336 (-1)  FN=448 (-9)   exact_start=81.79% (-1.68 pp)
  w=0.7: TP=1925 (+7)  FP=334 (-3)  FN=450 (-7)   exact_start=82.86% (-0.61 pp)
  w=0.8: TP=1935 (+17) FP=330 (-7)  FN=440 (-17)  exact_start=84.08% (+0.61 pp)

w=0.8 dominates baseline on every metric except |span|<=1 (basically
flat). Non-monotonic curve: w=0.5 over-aggressive, w=0.8 sweet spot,
w=0.7 partway down. w=0.9 tests whether the curve is still rising
at w=0.8 (gentler is even better) or whether w=0.8 is the peak.

Possible outcomes:
  A) w=0.9 even better: peak hasn't been hit; might try w=0.95 next.
  B) w=0.9 about equal to w=0.8: w=0.8 was the peak, accept w=0.8.
  C) w=0.9 worse than w=0.8 but still better than baseline: w=0.8 is
     the peak.
  D) w=0.9 collapses to baseline (no effect): expected at w=1.0; if it
     happens at w=0.9, the lever is functionally exhausted.
...
Decision rule:
  ...
  Accept and move on if better than baseline.

Output:
  Improvement_Snapshots/reach_detection/v8.0.0_dev_boundary_sample_weight_b1_w0.9/
"""
...
SNAPSHOT_DIR = Path(
    r"Y:\2_Connectome\Behavior\MouseReach_Pipeline\Improvement_Snapshots"
    r"\reach_detection\v8.0.0_dev_boundary_sample_weight_b1_w0.9"
)
...
def train_one_fold_with_boundary_weight(train_pool_df, train_video_ids, val_vid, feat_cols):
...
def main():
    ...
    print("  BSW w=0.8:   TP=1935  FP=330  FN=440  exact_start=84.08%  <-- best so far")
    print()
