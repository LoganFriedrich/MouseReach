"""
Phase E: validate Stage 3 in cascade with Stages 1 and 2.

Run Stage 1 first; for segments deferred, run Stage 2; for segments
deferred from Stage 2, run Stage 3. Compute per-stage trust +
cumulative coverage on the train pool.
"""
...
def main():
    print("=" * 70)
    print("PHASE E STAGE 3 VALIDATION (in cascade with Stages 1 + 2)")
    print("=" * 70)
    print()
    ...
    print(f"Running Stage 3 on {len(s3_inputs)} Stage-2-deferred segments ...",
          flush=True)
    cal3 = calibrate_stage(
        stage=stage3, seg_inputs=s3_inputs, gt_lookup=gt_lookup,
        okf_tolerance=3, ifr_tolerance=3, transition_zone_half=5)
    s3_committed = {(c.video_id, c.segment_num)
                    for c in cal3.cases if c.decision == "commit"}
    print(f"  Stage 3: {len(s3_committed)} committed, "
          f"{len(s3_inputs) - len(s3_committed)} deferred")
    print(f"  Stage 3 trust per class: {cal3.trust_per_class}")
    print()

    # --- Per-class breakdown for Stage 3 commits ---
    s3_commit_by_gt = defaultdict(int)
    s3_commit_by_gt_match = defaultdict(int)
    for c in cal3.cases:
        if c.decision != "commit":
            continue
        s3_commit_by_gt[c.gt_class] += 1
        if c.class_match and c.okf_within_tol:
            s3_commit_by_gt_match[c.gt_class] += 1

    print(f"Stage 3 commits by GT class (precision check):")
    for cls, n in s3_commit_by_gt.items():
        ok = s3_commit_by_gt_match.get(cls, 0)
        print(f"  {cls:>22s}: {ok}/{n} pass trust ({100*ok/n:.1f}%)")
    print()

    # Cumulative residual GT distribution
    s3_deferred = {(c.video_id, c.segment_num)
                   for c in cal3.cases if c.decision == "continue"}
    residual = defaultdict(int)
    for (vid, sn) in s3_deferred:
        gt = gt_lookup.get((vid, sn), {})
        residual[gt.get("gt_outcome")] += 1
    print(f"Cumulative residual after Stage 1 + Stage 2 + Stage 3 "
          f"({len(s3_deferred)} segments):")
    ...
    s2_untouched_correct = sum(
        ...
        if c.decision == "commit" and c.committed_class == "untouched"
        and c.class_match and c.okf_within_tol)
    s3_untouched_correct = sum(
        1 for c in cal3.cases
        if c.decision == "commit" and c.committed_class == "untouched"
        and c.class_match and c.okf_within_tol)
    cumulative = s1_untouched_correct + s2_untouched_correct + s3_untouched_correct
    print(f"Cumulative untouched yield: "
          f"S1={s1_untouched_correct} + S2={s2_untouched_correct} + "
          f"S3={s3_untouched_correct} = {cumulative} / {n_untouched_gt} "
          f"GT untouched ({100*cumulative/n_untouched_gt:.1f}%)")
    print()

    # Save
    out_dir = Path(
        r"Y:\2_Connectome\Behavior\MouseReach_Pipeline\Improvement_Snapshots"
        r"\outcome\v6.0.0_dev_stage3_validate")
    out_dir.mkdir(parents=True, exist_ok=True)
    (out_dir / "stage3_results.json").write_text(json.dumps({
        "stage1_n_committed": len(s1_committed),
        "stage2_n_committed": len(s2_committed),
        "stage3_n_committed": len(s3_committed),
        "stage3_commit_by_gt": dict(s3_commit_by_gt),
        "stage3_commit_by_gt_pass_trust": dict(s3_commit_by_gt_match),
        "cumulative_residual": dict(residual),
        "cumulative_untouched_yield": cumulative,
        "n_untouched_gt": n_untouched_gt,
    }, indent=2), encoding="utf-8")
    print(f"Saved: {out_dir / 'stage3_results.json'}")
