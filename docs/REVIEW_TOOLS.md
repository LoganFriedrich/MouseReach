...
- **It releases the video only when the routing reason names segmentation.** On save it writes `{stem}_deep_review_cleared.json` when the bundle's recorded routing reason mentions segmentation/mislabel -- fixing cuts is then exactly the work the video was routed for. A bundle routed on other grounds (a QC hold, an unrecorded reason) is deliberately NOT released by a cut-fix, because the clear marker is a blanket human-clear token the watcher gate honors; the status bar says so and the Deep Review tool's Clear (or `mousereach-review-release`) remains the path out once the real concern is resolved.
...
A new bundle is built out of sight and appears in one step. `move_video_bundle` (`review_routing.py`) moves the files into `<queue>/.incoming/<stem>/`, writes the routing manifest there, and then publishes the folder as `<queue>/<stem>/` with a single directory rename. **Why:** a move onto the shared drive is a copy that takes seconds per file, and the watcher's return scan reads the queues meanwhile. When the bundle folder was created first and filled in place, the scan saw it empty and filed it away mid-route (every remaining move then failed with "No such file or directory"), and a half-filled folder could be judged as a bundle before its files arrived. The readers that judge, retire or release bundles skip names starting with `.` (readers that key on stem-named files match nothing there by accident, not by rule), and the build folder sits under the queue itself so the rename never crosses a drive. A bundle that already exists (a re-route, or a divert into an existing bundle) is already visible and is filled in place as before. The routing manifest lists `moved_files` and `failed_files` (each file that could not be moved, with the error), so a part-moved bundle shows in the record rather than only in a log. A route that dies partway leaves its files in `.incoming/<stem>/` and the next route of that video reuses them; until then `mousereach-reconcile` lists the video as held, "being routed into <queue>, or an interrupted route -- check if this persists". If a concurrent route of the same video publishes the shared build folder first, the rest of this route's files and its routing manifest go into the published bundle, and a failed manifest write never stops the publish. A file that could not be merged in the fallback stays in `.incoming/<stem>/`, and the return refuses that bundle, naming the folder, until a person folds the file in -- returning would re-run the video without it (typically a locked mp4).

`reprocess_to_current._stage_review_bundle` (`reprocess_to_current.py:66`) stages bundles for the bring-current path and never clobbers a bundle that already exists (`:80-82`), but it does NOT build out of sight: it still creates `<queue>/<stem>/` first and fills it file by file, so a return scan can read it half-filled.

### How a video leaves

Only `scan_review_queues` (`review_return.py`) moves a bundle out. It runs inside the watcher every 10th poll cycle (`orchestrator.py:1345`, `:1391-1395`) and handles at most **10 bundles per scan** (`MAX_RETURNS_PER_SCAN`, `review_return.py`) so returning does not starve the pipeline.

Because the queues are shared storage scanned by every watcher node, every mutation the scan performs — a return, a stale-bundle retire, a divert to deep review — first takes a per-video claim marker in `{REVIEW_ROOT}/.return_claims/` (`_claim_return` / `_with_return_claim`, `review_return.py`). It is the same write-then-verify pattern as the DLC intake claims: write hostname+timestamp, wait 0.5s, re-read to confirm the write won. A stem another node holds is skipped this pass and re-examined next scan; a marker older than 2 hours is a crashed node's and is broken; with no shared root configured (single-node mode) the claim is a no-op. Scanning a bundle never takes a claim — only touching it does.

**An empty triage folder is left alone for 30 minutes before it is filed away.** An empty folder is not a bundle; it is residue of an earlier divert or return, and the scan retires it to `_Problematic` -- but only once its modification time is older than `EMPTY_DIR_GRACE_SECONDS` (30 minutes, `review_return.py`). **Why:** a router may have just created the folder and still be moving files in (routing into an existing bundle, or after a failed rename, fills the folder in place), and the time is stamped by the share's clock, which can differ from the scanning node's by many minutes. A time in the future, or one that cannot be read, counts as young. Real residue simply waits for a later scan. The same scan warns once per watcher run about any `<queue>/.incoming/<stem>/` older than 30 minutes and never touches it: those files are the video's only copy, and whether the route is still running or died is for a person to judge.
...
- **Triage with a failed segmentation**: the return scan diverts the bundle to deep review -- it can never satisfy the triage release condition (which requires a sound segmentation), and before this divert existed such bundles had no route out at all. The processing-side gate honors an existing human clear marker even when the segmentation self-check still reports failure, and since 2026-09-09 the return scan honors the same marker: a bundle carrying `{stem}_deep_review_cleared.json` is never diverted (the deep-review question is answered) and may release from triage once its triaged segments are answered, seg self-check notwithstanding. Before that, the gate's honor and the scan's raw `seg_failed` divert fought each other and a cleared video with unresolved triage segments lapped processing -> triage -> deep review -> processing indefinitely (one video made 43 laps in a day).

Returning moves the bundle's data files, including the review file, into the local processing directory, drops the queue-only metadata, deletes the now-empty bundle directory, and sets the video back to `processing` so the pipeline re-runs it and the gate re-checks (`_return_to_processing`). Queue-only metadata is matched by exact name -- `<stem>_manifest.json`, `<stem>_routing.json`, `manifest.json` -- so `<stem>_processing_manifest.json`, the video's provenance record, travels back with the bundle. **Why:** the old suffix match on `_manifest.json` caught the processing manifest too and deleted it from every returned bundle. A normal re-run regenerates it before the review gate; that regeneration is best-effort, though, so if it fails, or a segmentation failure routes the video before it runs, the old manifest stays beside the new outputs. The pose recorded for the re-run is the one `_resolve_inputs` selected (`select_pose_file`), not whichever `.h5` happened to move last. **Why:** a bundle can hold two poses (a re-pose leaves the old model's file beside the new one), and the last-moved rule re-ran such videos on the wrong, often older, pose. Gate-routed bundles carry their own mp4 and pose; only older staged bundles point at an mp4 and pose left in Analyzed. It refuses and leaves the bundle alone if files of the video are still in the queue's `.incoming/<stem>/` build folder, if the database row cannot be created, if the pose file cannot be found, or if the state cannot be set — a clearance is never spent on a run that would fail.
