...
A second periodic sweep sits beside this one (since 2026-08-25): the BenchDisagreementScanner (watcher/bench_disagreement.py, called from _maybe_bench_disagreement_scan, hourly by default, same handles_reprocessing gate). It compares each archived video's algorithm outcomes against the manual bench-sheet score and routes the video to the TRIAGE queue - with the specific segments flagged - when the bench sheet says a pellet was missed but the algorithm says it moved (physically, exactly one of the two is wrong) AND no review tool has ever answered that segment. "Never reviewed" is read off outcome_source == "algo", so a segment leaves this sweep's population permanently the moment any review answers it; no separate flagged-ledger exists, and a cleared triage does not get re-flagged even though the paper bench sheet never changes. The comparison itself does not run in the watcher's process at all: the watcher shells out to the MouseDB conda environment, which runs mousedb's never_reviewed_worklist recipe against the local parquet analysis snapshot (kept fresh by mousedb-refresh-snapshot, scheduled outside the watcher) - the watcher never opens connectome.db, which lives on a network share in rollback-journal mode where a blocked or torn read is a real risk. Ownership is decided by disk presence under Analyzed (a one-pass index of pellet_outcomes files), not by watcher.db state, which has documented divergence from disk; that also makes concurrent scans on multiple nodes safe, since the first node to route a video moves its files and later scans simply do not find them. This is the first retrospective caller of route_to_queue - every prior call site ran synchronously during first-pass processing.
