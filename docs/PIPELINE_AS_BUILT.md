...
Two roots. Every path in the system hangs off two settings. nas_root is the shared network folder everyone can see (Y:\LAB_ROOT\Behavior\MouseReach_Pipeline in this lab). processing_root is the machine's own fast local disk (A:\MouseReach_Pipeline on the GPU machines, C:\LAB_ROOT\Behavior\MouseReach_Pipeline on the analysis server). Folders under nas_root are shared; folders under processing_root are private to one machine.
...
Once a video is claimed, its files are copied - copied, not moved, so the handoff folder keeps a copy - into a folder named Processing on the SERVER'S OWN LOCAL DISK (on this machine, C:\LAB_ROOT\Behavior\MouseReach_Pipeline\Processing; the constant is built from the processing_root setting, src/mousereach/config.py:152). All algorithm work happens there. This is a different place from the network Processing zone on Y:, despite the identical folder name.
...
3. WHAT HAPPENS TO THE OLD FILES — they are overwritten, not archived
For a video marked outdated with a narrow scope, the second watcher copies every file whose name starts with the video identifier from Analyzed down to the local Processing folder — a copy, so the originals stay where they are (orchestrator.py:1574-1577). The algorithms re-run and overwrite the copies. Then the finished set is moved back to Analyzed/<project>/<cohort> by archive_video, which uses Python's shutil.move onto the same filename (archive/core.py:174-183). When that filename already exists, shutil.move silently replaces it. The result: the previous generation's segments, reaches, outcomes, assignments, triage record, manifest and kinematics file (<video id>_features.json) are destroyed by overwrite. Nothing is kept.
...
4. THE ARCHIVING CODE THE DESCRIPTION DESCRIBES DOES EXIST — nothing automatic calls it
...
2. The features file is pushed straight into the shared database from that working folder (watcher/orchestrator.py:2095-2096 and :1167-1168). The database is Y:/LAB_ROOT/Databases/connectome.db, table reach_data, one row per reach (sync/database.py:41).
