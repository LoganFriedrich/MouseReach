...
def main_batch():
    ...
    first-to-commit wins, triage on uncertain). Use --legacy to run the
    old geometric detector (core/pellet_outcome.py) instead.

    This command runs the SAME code path as the automatic pipeline:
    ``mousereach.outcomes.core.batch.process_batch`` -> ``process_single``,
    which is what the watcher, ``pipeline/run_all.py`` and
    ``pipeline/reprocess_to_current.py`` call per video. It has no
    detection or input-reading logic of its own.

    WHY: until 2026-08 this command carried its own copies of the input
    readers, and its reach reader only understood a flat top-level
    ``reaches`` list, while the reach detector (v8) nests reaches under
    ``segments[].reaches``. The cascade therefore ran here with ZERO reach
    windows -- every reach-dependent stage deferred and most touched
    segments fell through to triage -- while the pipeline, reading the
    same files through ``core/batch.py``, scored them. Two implementations
    of one step drift apart; there is now one.

    This command:
    1. Finds videos with reach detection results
    2. Tracks pellet position throughout each segment
    3. Classifies outcome (retrieved, displaced, untouched, or triaged)
    ...
    """
    from mousereach.outcomes.core.batch import process_batch
...
Examples:
  ...
  mousereach-detect-outcomes -i Processing/ -s '*_pellet_outcomes.json'
    ...
    parser.add_argument('--legacy', action='store_true',
                        help="Run the legacy geometric detector "
                             "(core/pellet_outcome.py) instead of the v6 cascade")
    args = parser.parse_args()

    if not args.quiet:
        if args.legacy:
            from mousereach.outcomes.core.pellet_outcome import VERSION as LEGACY_VERSION
            print(f"[outcome detector] legacy geometric detector (VERSION {LEGACY_VERSION})")
        else:
            from mousereach.outcomes.v6_cascade import VERSION
    ...
    if args.quiet:
        # process_batch prints the same line itself when verbose.
