...
def _build_points_array(df: pd.DataFrame, bodyparts: List[str],
    ...
    """Build napari-ready (N, 3) points array and (N, 4) RGBA colours."""
    ...
    for frame_idx in range(n_frames):
        ...
        for bp in bodyparts:
            ...
            lk = row.get(f'{bp}_likelihood', 0)
            if lk > LIKELIHOOD_THRESHOLD and not np.isnan(x) and not np.isnan(y):
                points.append([frame_idx, y, x])
                colors.append(bp_colors[bp] + [alpha])
        ...
        if self.df_b is not None:
            ...
            if len(pts) > 0:
                self.points_layer_b = self.viewer.add_points(
                    pts, name='Model B', size=4, face_color=cols,
