...
`Y:\LAB_ROOT\Validation_Runs\<trigger>_<date>\reports\`. Keep them.
...
2. Run the new (recalibrated) MouseReach on those videos, writing
   ...
   `Y:\LAB_ROOT\Validation_Runs\<trigger>_<date>\generalization\`).
...
`Y:\LAB_ROOT\Validation_Runs\DLC_2026_03_27\`. That directory is a
