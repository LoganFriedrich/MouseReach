...
|--------|----------|---------|
| Per-reach Sankey v2 | `...outcome_master_pre_v4.0.0_baseline\figures\sankey_per_reach.png` | `...v6_cascade_2026-05-04\figures\sankey_per_reach.png` |

**v2 renderer** (2026-05-04): Both Sankeys now use the same canonical renderer with:

1. **All canonical categories zero-padded on both sides** so two Sankeys compare without layout shifts.
2. **Triaged propagates to ALL reaches in triaged segments** (not just the would-be-causal reach). Conservative: every reach in a triaged segment is routed to manual review.
3. **`wrong_reach_called`** (algo-side only): derived category for reaches where the algo committed the correct outcome class but attributed it to a different physical reach than GT's causal reach.

Category definitions (both sides unless noted):

| Category | Side | Meaning |
|----------|------|---------|
| `retrieved` | both | Causal reach in a retrieved segment |
| `displaced_sa` | both | Causal reach in a displaced segment |
| `untouched` | both | (always 0 on per-reach -- untouched segments have no causal reach) |
| `abnormal_exception` | both | Causal reach in a non-evaluable segment (tail-knockover etc.) |
| `miss` | both | Non-causal reach (happened but didn't cause the outcome) |
| `triaged` | both | Reach in a segment flagged for manual review |
| `absent` | both | Reach exists only on the other side (FP/FN from reach detector) |
| `wrong_reach_called` | algo only | Algo committed correct class but attributed to wrong physical reach |

Headline per-reach counts (CURRENT v6 cascade):
- Total reaches: 5368
- `triaged` (algo side): 798 reaches (all reaches across 97 triaged segments)
- `wrong_reach_called` (algo side): 11 reaches
- `miss__triaged` flow: 743 (non-causal reaches in triaged segments)

Headline per-reach counts (BASELINE v3.1):
- Total reaches: 6682
- `triaged`: 0 (baseline had no triage)
- `wrong_reach_called`: 0 (baseline used different reach matcher)
...
| Version | Per-segment Sankey | Per-reach Sankey (v2) |
|---------|--------|--------|
| **BASELINE ("v3.1" / `master_pre_v4.0.0`)** | n/a (single-classifier, no triage) | `Y:\...\outcome_master_pre_v4.0.0_baseline\figures\sankey_per_reach.png` |
| **CURRENT (v6 cascade)** | `Y:\...\v6_cascade_2026-05-04\figures\sankey.png` | `Y:\...\v6_cascade_2026-05-04\figures\sankey_per_reach.png` |
