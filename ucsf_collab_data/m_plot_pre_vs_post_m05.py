"""
M05: Pre-injury top reaches vs Post-injury top reaches side by side.
"""
...
def plot_reach(ax, dlc, s, e, label):
    ...
    ax.text(0.02, 0.02, 'lk=%.2f' % np.mean(rh_lk),
...
def main():
    targets = [
        # Row 1: M05 pre-injury top 4
        ('PRE', 'M05-44972-P2', 9076, 9089, 488.8),
        ('PRE', 'M05-44972-P2', 18129, 18140, 460.5),
        ('PRE', 'M05-44972-P1', 11674, 11690, 449.4),
        ('PRE', 'M05-44972-P1', 15711, 15736, 436.8),
        # Row 2: M05 post-injury top 4
        ('POST', 'M05-45001-P1', 20268, 20284, 812.7),
        ('POST', 'M05-45001-P1', 7254, 7273, 791.7),
        ('POST', 'M05-45001-P1', 25975, 25991, 773.7),
        ('POST', 'M05-45001-P2', 7105, 7126, 762.8),
    ]
    ...
    for idx, (phase, session, s, e, area) in enumerate(targets):
        ...
        dur = e - s + 1
        label = 'M05 %s  area=%.0f  dur=%d\n%s frames %d-%d' % (
            phase, area, dur, session, s, e)
        print('Plotting %s %d-%d area=%.0f' % (session, s, e, area))
        plot_reach(axes[idx], dlc, s, e, label)

    fig.suptitle('M05: Pre-Injury Top Reaches (row 1) vs Post-Injury Top Reaches (row 2)\n'
                 'Same animal, same plotting. Are post-injury reaches genuinely larger?',
                 fontsize=13, fontweight='bold')
    plt.tight_layout()
    out = os.path.join(OUTPUT_BASE, 'M05_pre_vs_post.png')
